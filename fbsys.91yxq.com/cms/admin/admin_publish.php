...
switch($IN[o]) {
		...
		case 'content_list': //内容管理 文章列表
			...
			//require_once dirname( realpath(ROOT_PATH) ).'/api/get_gamelist2.php';
			if(!empty($IN['IndexID'])) {
			...
			// print_r($NodeInfo);
			// echo "</pre>";
			//print_r($NodeInfo['TableID']);
			// $mysqli = new mysqli($db_config['db_host'],$db_config['db_user'],$db_config['db_password'],$db_config['db_name']);
			// $mysqli->set_charset('UTF8');
			// $game_info = $mysqli->query("SELECT a.PlatformId FROM xxxwan_content_6 a,xxxwan_content_index b WHERE b.NodeID={$NodeInfo['NodeID']} AND b.ContentID=a.ContentID LIMIT 1")->fetch_row();
			//$mysqli->close();
			if( $NodeInfo['TableID'] == 6 ){
				require_once dirname( realpath(ROOT_PATH) ).'/api/get_gamelist2.php';
				$platform_info = $db->getRow( "SELECT c.PlatformId FROM {$db_config[table_pre]}content_6 c inner join {$db_config[table_pre]}content_index i on i.ContentID=c.ContentID and i.TableID=6 WHERE i.NodeID={$NodeInfo[NodeID]} LIMIT 1" );
				$TPL->assign("game_arr", $game_arr[$platform_info['PlatformId']]);
			}
			elseif( $NodeInfo['TableID'] == 7 ){
				require_once dirname( realpath(ROOT_PATH) ).'/api/get_gamelist2.php';
				$platform_info = $db->getRow( "SELECT c.PlatformId FROM {$db_config[table_pre]}content_7 c inner join {$db_config[table_pre]}content_index i on i.ContentID=c.ContentID and i.TableID=7 WHERE i.NodeID={$NodeInfo[NodeID]} LIMIT 1" );
				$TPL->assign("game_arr", $game_arr[$platform_info['PlatformId']]);
			}else{
				require_once dirname( realpath(ROOT_PATH) ).'/api/get_gamelist2.php';
				$TPL->assign("game_arr", $game_arr);
			}
			if( $NodeInfo['TableID'] == 5 || $NodeInfo['TableID'] == 6 || $NodeInfo['TableID'] == 7 ){
				$platform_arr = include(dirname( realpath(ROOT_PATH) ).'/api/platformDefined.php');
				$TPL->assign("platform_arr", $platform_arr );
			}
		...
		case 'add':	//生成内容添加的UI,已经转至admin_editor.php处理
			...
			// $mysqli = new mysqli($db_config['db_host'],$db_config['db_user'],$db_config['db_password'],$db_config['db_name']);
			// $mysqli->set_charset('UTF8');
			// $game_info = $mysqli->query("SELECT MAX(a.ServerId) as ServerId,a.PlatformId,a.GameId FROM xxxwan_content_6 a,xxxwan_content_index b WHERE b.NodeID={$NodeInfo['NodeID']} AND b.ContentID=a.ContentID LIMIT 1")->fetch_assoc();
			$game_info = $db->getRow( "SELECT MAX(a.ServerId) as ServerId,a.PlatformId,a.GameId FROM {$db_config[table_pre]}content_6 a,{$db_config[table_pre]}content_index b WHERE b.NodeID={$NodeInfo[NodeID]} AND b.ContentID=a.ContentID LIMIT 1" );
			$game_info && $pInfo = array(
				'ServerId' => $game_info['ServerId'] + 1,
				'ServerName' => '双线' . ($game_info['ServerId'] + 1) . '服',
				'GameId' => $game_info['GameId'],
				'PlatformId' => $game_info['PlatformId'],
				'ServerStatus' => 2, //待开
				'ServerShow' => 3, //全部显示
				'MergeId' => 0,
			);
		...
		case 'add_submit':
            ...
            //游戏列表每添加新的游戏，则自动增加独立游戏攻略结点
            if ($IN['NodeID'] == 7524){
                include dirname(__DIR__). '/api/BuildNodeID.php';
            } 
            if ($IN['NodeID'] == 7554){
                include dirname(__DIR__). '/api/RefreshNodeID.php';
            }
			$publish->flushData();
		...
		case 'edit':
			require_once INCLUDE_PATH."admin/TplVarsAdmin.class.php";
			if(empty($IN[IndexID])) fgoto('content_list');
		...
		case 'del':
			...
			if(!empty($IN[multi]) && !empty($IN[pData]) ) {
				...
				if($result) {
					if($NodeInfo[TableID]==6){
					    // 推送服务器列表到平台
						 require_once dirname( realpath(ROOT_PATH) ).'/api/updateServerData.php';
						 //require_once '../../api/updateWdServer.php';
						 echo '请刷新 开服列表、首页，如新增服务器，请刷新 新手卡页';
					}
		...
		case 'copy':
			...
			if(!empty($IN[multi]) && !empty($IN[pData]) ) {
				...
				if($result) {
                    ...
                    if($IN['targetNodeID'] == 7554)
                        include_once dirname(__DIR__)."/api/UpdateNodeID.php";//更新手机端独立游戏攻略结点
		...
		case 'planPublish':
			...
			break;
 			...
 			case 'set_server_status':   // added by Max 20110507
			include_once SETTING_DIR ."cms.ini.php";
			//debug($IN);
			if(!empty($IN[multi]) && !empty($IN[pData]) ) {
				foreach($IN[pData] as $var) {
					$result = $publish->setServerStatus($var,$IN['status']);
				}

				if($result) {
					Task::refreshIndex($NodeInfo[NodeID]);
					showmessage('server_set_ok', $referer);

				}
				else
					showmessage('server_set_fail', $referer);
			}
		    break;
