...
class Action{
    ...
    protected function getGuildMembers($p, $w = array()){
        ...
        $this ->model ->fields = array('site_id', 'author');
    ...
    //短链接【备用】
    protected function getShotUrl1($reg) {
        $ch=curl_init();
        ...
        if($arrResponse['status']!=0){
               $url=file_get_contents('http://tinyurl.com/api-create.php?url='.$reg);
               return $url;
        }else{
                return $arrResponse['tinyurl'];  
        }
...
//短链接
    ...
    protected function get_info($url, $param =array()){
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_GET, 1);
