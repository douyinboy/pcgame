...
if($_POST['action']){//注册
    ...
    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,urlencode($adid),'',1,'',$pcid, 'tiantianzuan');
    if ($result == 'ok') {
        $url = "http://ttz.api.china.com/api/get_userid.php?user_name=" . urlencode(strtolower($login_name));
        $result = get_curl($url);
        $userId   = $result;
        $userName = $login_name;
        $keycode = strtolower(md5($pcid . $adid . $key));

        $url = 'http://api.ttz.com?api_action=Game.Api.Games&project_id='.$adid.'&regid='.$pcid.'&uid='.$userId.'&username='.$userName.'&sign='.$keycode;

        $res = get_curl($url);
        if ($res != 0) {
            exit('天天钻绑定失败----' . $res);
        } else {
            file_put_contents('tiantianzuan.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
