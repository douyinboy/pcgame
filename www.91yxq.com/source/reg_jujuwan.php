...
//    $key = 'd29c1fe77841a8a2b5bf7dbd607a24e0';
    ...
    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,urlencode($adid),'',1,'',$pcid, 'jujuwan');
    if ($result == 'ok') {
        $url = "http://api.91yxq.com/api/get_userid.php?user_name=" . urlencode(strtolower($login_name));
        $result = get_curl($url);
        $userId   = $result;
        $userName = $login_name;
        $sign = strtolower(md5($userId . $pcid . 'jujuwan'));
//regid=&uid=&uname=&key=?
        $url = 'http://jjw.huixiangyou.com/returns/JJWReturn.ashx?regid='.$pcid.'&uid='.$userId.'&uname='.$userName.'&key='.$sign;

        $res = get_curl($url);
        $res2 = json_decode($res, true);

        if ($res2['result'] != 1) {
            exit('聚聚玩绑定失败----' . $res2['msg']);
        } else {
            file_put_contents('jujuwan.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
