...
class userAdmin extends iData
{
				...
				public function getAllByPermission( )
				{
								...
								{
									//增加  order by g.gId
												$sql = "SELECT u.*,g.gName,u2.uName as CreationUserName FROM {$table->user} u left join {$table->group} g ON g.gId=u.uGId left join {$table->user} u2 ON u.CreationUserID=u2.uId order by g.gId";
								}
								else
								{
				...
				public function Counter( $uId, $field, $op = "+", $type = 0, $NodeInfo = "" )
				{
								global $table;
								global $db;
								global $db_config;
				...
				public function canAccess( $uId, $mode )
				{
								global $group;
								global $sys;
								if ( !isset( $group ) )
				...
				public function PermissionDetector( &$IN )
				{
								global $_LANG_ADMIN;
								global $sys;
								global $group;
