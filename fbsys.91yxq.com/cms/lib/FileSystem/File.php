...
class FileSystem_File
{
				...
				public function xdel( $_path, $_filename )
				{
								if ( substr( $_path, -1 ) == "/" )
								{
				...
				public function xmkdir( $_path, $_mode = 511 )
				{
								$fullpath = "";
				...
				public function automkdir( $_path, $_mode = 511 )
				{
								global $SYS_CONFIG;
				...
				public function autowrite( $_filename, &$_content )
				{
								global $SYS_CONFIG;
								if ( $SYS_CONFIG['ftp_mode'] === 1 )
								{
				...
				public function autocopy( $_src, $_des )
				{
								global $SYS_CONFIG;
								if ( $SYS_CONFIG['ftp_mode'] === 1 )
								{
				...
				public function _ftp_write( $_filename, &$_content, $SYS_CONFIG )
				{
				...
				public function _ftp_upload( $_src, $_des, $SYS_CONFIG )
				{
								$conn_id = ftp_connect( $SYS_CONFIG['ftp_host'], $SYS_CONFIG['ftp_port'] );
								ftp_login( $conn_id, $SYS_CONFIG['ftp_username'], $SYS_CONFIG['ftp_password'] );
				...
				public function xtmpfile( &$content )
				{
				...
				public function _ftp_realpath( $_root_path, $_path )
				{
				...
				public function _ftp_mkdir( $directory, $SYS_CONFIG, $mode = 511 )
				{
								$mode = $mode == "" ? 511 : $mode;
								$mode = decoct( $mode );
								if ( strlen( $mode ) == 4 )
				...
				public function write( $_filename, $_content, $_mode = 511 )
				{
				...
				public function copyDir( $_src_dir, $_des_dir )
				{
								if ( $dir = dir( $_src_dir ) )
								{
												$dir->rewind( );
