var login_tips = {
    ...
    verify : function(){
        if(document.getElementById('login_verify')==null){
            ...
            login_verify_html += '<p style="margin:0;padding:0;text-align:center;"><img id="login_verify_img" src="http://www.6qwan.com/chkcode.inc.php" onclick="this.src=\'http://www.6qwan.com/chkcode.inc.php?\'+Math.random();" alt="验证码,看不清楚?请点击刷新验证码" style="cursor:pointer;margin:5px;" /></p>';
    ...
    check_verify : function(){
        ...
        $.getJSON('http://www.6qwan.com/api/check_login_verify.php?verify_code='+verify_code+'&callback=?',function (data){
            if(data){
                if(data.code=='10'){
