...
class cate_tpl_admin extends iData
{
				...
				public function copyTo( $TID, $TCID )
				{
								global $table;
								global $sys;
								global $SYS_ENV;
				...
				public function getTplContent( $TID )
				{
								global $db;
								global $table;
				...
				public function getAllbyType( $TCID, $type )
				{
								global $table;
								global $db;
				...
				public function addTpl( $TCID, $tpltype, $tplname, $tplcontent )
				{
								global $db;
								global $tplname;
								$this->addData( "TCID", $TCID );
				...
				public function writeTPL( &$TCID, &$tplname, &$content )
				{
				...
				public function ftpMakeDir( $directory, &$conn_id )
				{
								$pwd = ftp_pwd( $conn_id );
				...
				public function _db_update( $TID )
				{
								global $table;
								$where = "where TID=".$TID;
								if ( $this->dataUpdate( $table->tpl_data, $where ) )
				...
				public function tplExist( $TCID, $tplname )
				{
								global $table;
								global $db;
