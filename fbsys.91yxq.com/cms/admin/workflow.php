...
switch($IN[o]) {
	...
	case 'add_submit':
		...
		if($workflow->add()) { 
			...
			fgoto("view", 'add_workflow_ok');

		} else {
			fgoto("view", 'add_workflow_fail');
		
		}

		break;
	case 'edit':
		if(empty($IN[wID])) fgoto('view');
	...
	case 'edit_submit':
		...
		if($workflow->update($IN[wID])) { 
 			fgoto("view", 'edit_workflow_ok');

		} else {
			fgoto("view", 'edit_workflow_fail');
	...
	case 'del':
		...
		if($workflow->del($IN[wID])) {
			fgoto("view", 'del_workflow_ok');
			
		} else
			fgoto("view", 'del_workflow_fail');
	...
	case 'add_state_submit':
		...
		if($workflow->add_state()) { 
			...
			fgoto("view", 'add_workflow_state_ok');

		} else {
			fgoto("view", 'add_workflow_state_fail');
		
		}

		break;
	case 'edit_state':
		if(empty($IN[ID])) fgoto('view');
	...
	case 'edit_state_submit':
		...
		if($workflow->update_state($IN[ID])) { 
 			fgoto("view", 'edit_workflow_state_ok');

		} else {
			fgoto("view", 'edit_workflow_state_fail');
	...
	case 'del_state':
		...
		if($workflow->del_state($IN[ID])) {
			fgoto("view", 'del_workflow_state_ok');
			
		} else
			fgoto("view", 'del_workflow_state_fail');
