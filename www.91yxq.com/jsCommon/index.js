...
script.src = 'http://www.6qwan.com/jsCommon/login_tips.js';
...
function CheckInput(){
    if(getCookie('last_name')){
       ...
       // pass = unescape(getCookie('last_pass')).replace("+"," ");
		...
		$('#login_user').val('请输入6qwan帐号');		
        $('#login_user').focus(function(){
            if($(this).val() != '请输入6qwan帐号'){
				 return;
			}
            $(this).val('');
			$('#login_pwd').val('');
...
function CheckLogined(){
    ...
    if(login_name){
        ...
        if(qq_nick&&login_name.indexOf('@qq')>0){
            ...
            //gID('user_pic').src='http://bbs.6qwan.com/uc_server/avatar.php?uid='+userid+'&size=middle';
    ...
    if(login_name){
        ...
        if(qq_nick&&login_name.indexOf('@qq')>0){
           // gID('username').innerHTML='您好！'+qq_nick+' - '+login_name;
            ...
            var a = user_info.split('|');
            var userid = a[0];
            //gID('user_pic').innerHTML='<img src="http://bbs.6qwan.com/uc_server/avatar.php?uid='+userid+'&size=middle" width="" height="44" />';
        ...
        if(login_game_info != 0){
			...
			if(gameList.length>0){
				for(var i =0;i < gameList.length;i++){
					gameinfo = gameList[i].split("_");
					key=0;
					playedGames += "<a class='gre' href='http://www.6qwan.com/main.php?act=gamelogin&game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' target='_blank'><span>"+gameinfo[key+2]+"</span></a> <a href='http://pay.6qwan.com/?game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' class='black' target='_blank'>[充值]</a> | ";
				}
			}
    ...
    if(login_name){
        ...
        if(qq_nick&&login_name.indexOf('@qq')>0){
           // gID('username').innerHTML='您好！'+qq_nick+' - '+login_name;
            ...
            var a = user_info.split('|');
            var userid = a[0];
            //gID('user_pic').innerHTML='<img src="http://bbs.6qwan.com/uc_server/avatar.php?uid='+userid+'&size=middle" width="44" height="44" />';
            gID('logined_username').innerHTML='<span class="fl"><b>尊敬的 <a href="http://www.6qwan.com/my.php">'+login_name.replace("+"," ")+'</a></b> 你好！</span><span class="fr"><a href="http://www.6qwan.com/main.php?act=logout&href=/dpqk/">退出</a></span><div class="clearfix"></div>';
        }		
        ...
        if(login_game_info != 0){
			...
			if(gameList.length>0){
				for(var i =0;i < gameList.length;i++){
					gameinfo = gameList[i].split("_");
					key=0;
					playedGames += "<span><a class='gre' href='http://www.6qwan.com/main.php?act=gamelogin&game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' target='_blank'>"+gameinfo[key+2]+"</a> | <a href='http://pay.6qwan.com/?game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' class='black' target='_blank'>[充值]</a></span>";
				}
			}
...
function Login(){
    ...
    if( login_user == '' || login_user == '请输入6qwan帐号'){
...
function copy_code(){
    ZeroClipboard.setMoviePath( 'http://image.6qwan.com/images/ZeroClipboard.swf' );
