...
if($_POST['action']){//注册
    ...
    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,urlencode($adid),'',1,'',$pcid, 'jiquwang');
    if ($result == 'ok') {
        $url = "http://api.91yxq.com/api/get_userid.php?user_name=" . urlencode(strtolower($login_name));
        $result = get_curl($url);
        $userId   = $result;
        $userName = $login_name;
        $sign = strtolower(md5($pcid . $adid . $userId . $userName . $server_id . $key));

        $url = 'http://www.ji7.com/gameapi/bind?ji7_uid='.$pcid.'&ji7_adid='.$adid.'&u_code='.$userId.'&u_name='.$userName.'&sign='.$sign.'&serverid='.$serverid;

        $res = get_curl($url);
        $res2 = json_decode($res, true);

        if ($res2['status'] != 0) {
            exit('集趣网绑定失败----' . $res2['msg']);
        } else {
            file_put_contents('jiquwang.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
