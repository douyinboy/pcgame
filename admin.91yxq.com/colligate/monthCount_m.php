...
if (!$date1) {
	$date1=date("Y-m",time()-3*30*86400);
} 
if (!$date2) {
	$date2=date("Y-m");
}
...
$s=" WHERE  `pay_date`>='".$date1."-01 00:00:00' AND `pay_date`<='".$date2."-31 23:59:59' $f";
...
<script language="javascript" type="text/javascript">
    function game_change(info) {
        if(!info){return false;}
            ...
            var date1 = $("#date1").val();
            var date2 = $("#date2").val();
        window.location.href='monthCount_m.php?game_id='+info+'&user_name='+uname+'&date1='+date1+'&date2='+date2;
   }
   function btnsubmit() {
   $("#myform").submit();
   }
...
<TABLE class=table_list border=0 cellSpacing=1 cellPadding=3 width="100%" height=32>
  ...
  <TR class=trEven>
    <TD>
      ...
      &nbsp;&nbsp;充值日期：
      <INPUT  value="<?=$date1?>" name="date1" id="date1" size="20" onClick="WdatePicker({dateFmt:'yyyy-MM'});" > -
	  <INPUT  value="<?=$date2?>" name="date2" id="date2" size="20" onClick="WdatePicker({dateFmt:'yyyy-MM'});" >
      &nbsp;&nbsp;游戏：
...
<TABLE class=table_list border=0 cellSpacing=1 cellPadding=2>
  ...
  <?php 
    ...
    $sql="select distinct DATE_FORMAT( pay_date, '%Y-%m' ) as tmon from ".PAYDB.".".PAYLIST." $s AND game_id !=0 AND server_id !=0 order by tmon";
    $pay_mon_arr = $db ->find($sql);
    $data1=array();
    $data2=array();
    $i=0;
    foreach ($pay_mon_arr as $val) {
        $tmon=$val['tmon'];	
        $sql="select count(distinct user_name) as pn from ".PAYDB.".".PAYLIST." $s  AND game_id !=0 AND server_id !=0  and DATE_FORMAT( pay_date, '%Y-%m' )='$tmon'";
        $pre =$db->get($sql);

        $sql="select sum($pay_field) as sm,pay_way_id from ".PAYDB.".".PAYLIST." $s  AND game_id !=0 AND server_id !=0 and DATE_FORMAT( pay_date, '%Y-%m' )='$tmon' group by pay_way_id";
        $pay_m_arr =$db -> find($sql);

        //获取所有人工充值
        //首充
        $stime = strtotime($tmon."-01 00:00:00");
        $etime = strtotime(date('Y-m-d', strtotime($tmon."-01 +1 month -1 day"))." 23:59:59");
    ...
    //公会赔付
        ...
        foreach ($pay_m_arr as $val2) {
            ...
            $paysum1+=$val2['sm'];
            $paysum[$val2["pay_way_id"]]+=$val2['sm'];
        }
        $pay[11] +=$totalrg;
  ...
  ?>
    <TR class=<?php if ($i%2==0) echo 'trOdd'; else echo 'trEven';$i++;?>>
        <TD noWrap><?=$tmon?></TD>
    <?php foreach ($pay_way_arr as $kk =>$vv){ ?> <!--充值渠道-->
