...
//	}
    ...
    $key = '15b822682cdeb4f2dc11816b2e9dbb69';

    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,urlencode($adid),'',1,'',$pcid, 'youyiwang');
    if ($result == 'ok') {
        $url = "http://api.91yxq.com/api/get_userid.php?user_name=" . urlencode(strtolower($login_name));
        $result = get_curl($url);
        $userId   = $result;
        $userName = $login_name;
        $keycode = md5($pcid . $userId . $adid . $key);

        $url = 'http://www.yuu1.com/index.php?tp=tryapi&gid='.$adid.'&uid='.$pcid.'&guid='.$userId.'&gname='.$userName.'&sign='.$keycode;

        $res = get_curl($url);
        if ($res != 0) {
            exit('优易网绑定失败----' . $res);
        } else {
            file_put_contents('youyiwang.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
