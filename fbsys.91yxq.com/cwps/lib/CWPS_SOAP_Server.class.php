...
class CWPS_SOAP_Server extends iData
{
				...
				public $SyncOrderRelationReqTpl = "";
				public $SyncOrderRelationRespTpl = "";
				public $Request = array( );
				public $Response_hRet = 1;
				public $Response_FeatureStr = "";
				public $Response_Return = "";
				public $TransactionAccessKey = "";
				public $TransactionAccessKeyFileName = "";
				public $ActionReqData = "";
				public $Version = "1.5";
				public $Valid_OAS_IP = array( );
				public $Valid_Action = array( );
				public $DataEncode = true;
				public $Response_Charset = "utf8";
				public $transferEncrypt = false;

				public function CWPS_SOAP_Server( )
				{
								$this->soapInterfacePath = SOAP_INTERFACE_PATH;
								$this->Response_hRet = SOAP_UNKNOWN_ERROR;
								$this->ActionRespTpl = "<?xml version=\"1.0\" encoding=\"utf-8\" ?><SOAP-ENV:Envelope  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:SOAP-ENC=\"http://schemas.xmlsoap.org/soap/encoding/\"><SOAP-ENV:Header>    <TransactionID xmlns=\"http://www.cmsware.com/passport/schemas/\">{TransactionID}</TransactionID>    <TransactionEncrypt xmlns=\"http://www.cmsware.com/passport/schemas/\">{TransactionEncrypt}</TransactionEncrypt></SOAP-ENV:Header><SOAP-ENV:Body><ActionResp xmlns=\"http://www.cmsware.com/passport/schemas/\"><Version>".$this->Version."</Version>"."<Encoding>{Encoding}</Encoding>"."<hRet>{hRet}</hRet>"."<RespCharset>{RespCharset}</RespCharset>"."<FeatureStr>{FeatureStr}</FeatureStr>"."<Return>{Return}</Return>"."</ActionResp>"."{SOAP-ENV:Fault}"."</SOAP-ENV:Body>"."</SOAP-ENV:Envelope>";
				...
				public function addResponseElement( $key, $value )
				{
								if ( is_array( $value ) )
								{
				...
				public function array2element( $data )
				{
				...
				public function setEncode( $_encode = false )
				{
				...
				public function addOAS( $OASID, $ip, $pass )
				{
								if ( strpos( $ip, "," ) !== false )
								{
				...
				public function soap_encode( $str )
				{
								if ( $this->transferEncrypt )
								{
				...
				public function soap_decode( $str )
				{
								if ( $this->transferEncrypt )
								{
				...
				public function parseActionReq( &$str )
				{
				...
				public function getTransactionAccessKey( )
				{
								return $this->TransactionAccessKey;
				}

				public function initDbResult( )
				{
								global $db;
								global $db_config;
