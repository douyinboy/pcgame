...
switch($_POST['do']){
	case "reg":   //账号注册
		...
		$nowtime = time();
		$res = $mysqli->query("INSERT INTO `users`(user_name, openid, agent_id, place_id, reg_time) VALUES('".$_POST['username']."', '".$_POST['openid']."', ".intval($_POST['agent_id']).", ". $mysqli->escape_string(strip_tags(trim($_POST['placeid'] . ''))).", $nowtime)");
		$uid = $mysqli->insert_id;
		if($uid<=0) {
		    echo "no";exit;
		}

        $res_pceggs = $mysqli->query("INSERT INTO `pceggs`(uid, pcid, game_id, server_id, adid, create_time, update_time) VALUES(".$uid.", ".intval($_POST['pcid']).", ".intval($_POST['game_id']).", ".intval($_POST['server_id']).", '".$_POST['adid']."', ".$nowtime.", ".$nowtime.")");
  ...
  break;
	case "checkopenid":
        $rc = $mysqli->query("select user_name from `users` WHERE `openid`='".$_POST['openid']."'")->fetch_assoc();
        echo $rc['user_name'];
        break;
  case "getpwd":
