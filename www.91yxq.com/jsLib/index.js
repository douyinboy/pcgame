...
script.src = 'http://www.demo.com/jsLib/login_tips.js';
        ...
        if(login_game_info != 0){
			...
			if(gameList.length>0){
				for(var i =0;i < gameList.length;i++){
					gameinfo = gameList[i].split("_");
					key=0;
					playedGames += "<a class='gre' href='http://www.demo.com/turn.php?act=gamelogin&game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' target='_blank'><span>"+gameinfo[key+2]+"</span></a> <a href='http://pay.demo.com/?game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' class='black' target='_blank'>[充值]</a> | ";
				}
			}
    ...
    if(login_name){
        ...
        if(qq_nick&&login_name.indexOf('@qq')>0){
           // gID('username').innerHTML='您好！'+qq_nick+' - '+login_name;
            ...
            var a = user_info.split('|');
            var userid = a[0];
            gID('logined_username').innerHTML='<span class="fl"><b>尊敬的 <a href="http://www.demo.com/user.php">'+login_name.replace("+"," ")+'</a></b> 你好！</span><span class="fr"><a href="http://www.demo.com/turn.php?act=logout&href=/dpqk/">退出</a></span><div class="clearfix"></div>';
        }		
        ...
        if(login_game_info != 0){
			...
			if(gameList.length>0){
				for(var i =0;i < gameList.length;i++){
					gameinfo = gameList[i].split("_");
					key=0;
					playedGames += "<span><a class='gre' href='http://www.demo.com/turn.php?act=gamelogin&game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' target='_blank'>"+gameinfo[key+2]+"</a> | <a href='http://pay.demo.com/?game_id="+gameinfo[key]+"&server_id="+gameinfo[key+1]+"' class='black' target='_blank'>[充值]</a></span>";
				}
			}
...
function CheckTopLogined(){
    var login_name=unescape(getCookie('login_name'));
    if(login_name){
        gID('top_login').innerHTML="欢迎您,<a href='/user.php' style='color:#090909'><strong>"+login_name+"</strong></a>, <a href='/turn.php?act=logout'>注销</a>";
    }
}
