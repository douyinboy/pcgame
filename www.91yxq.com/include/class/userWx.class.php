...
///////////////////////一下是不需要外面掉用的/////////////////////////////////////////
	...
	function login_($user,$pows,$my=false,$game=false){
		...
		if(!$user or !$pows) {
			...
			$results=long_login($info, time(),'auto_login');
		}
...
// 			$game_str=escape(iconv('UTF-8', 'GBK', $result_arr[11]));
	...
	/*用户注册*/
	function wxReg($username,$pows,$openid,$mail,$sex,$id_card,$question='',$answer='',$agent_id,$game_id,$server_id='0',$placeid='',$from_url='',$adid='',$truename='',$rand='1',$cplaceid='',$pcid){
		GLOBAL $cookiepath,$cookiedomain;
		$uid=$uid?$uid:0;
		if ($from_url=='') {
			$from_url = $_COOKIE['__from_url__'];
		}
		$info='username='.$username.'&passwd='.md5($pows).'&openid='.$openid.'&email='.$mail.'&ip='.GetIP().'&agent_id='.$agent_id.'&game_id='.$game_id.'&from_url='.$from_url.'&placeid='.$placeid.'&adid='.$adid.'&server_id='.$server_id.'&truename='.$truename.'&idcard='.$id_card.'&rand='.$rand.'&cplaceid='.$cplaceid.'&pcid='.$pcid;//定义传递规则

		$sty_states=long_login($info, time(),'wx_reg&do=reg');
