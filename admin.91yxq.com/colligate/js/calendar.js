...
L_calendar.prototype = {
    ...
    CreateHTML: function() {
        ...
        for (var i = 0; i < this.L_WDay.length; i++) {
            htmlstr += "<li id=\"L_calendar-day_" + i + "\" style=\"background:#F3F3FF\" onmouseover=\"this.style.background='#FFD700'\"  onmouseout=\"this.style.background='#F3F3FF'\"></li>\r\n";
        }
        htmlstr += "</ul>\r\n";
        ...
        stylestr += "body{background:#fff;font-size:12px;margin:0px;padding:0px;text-align:left;font-family:\"微软雅黑,宋体,Verdana\"}\r\n";
        stylestr += "#L_calendar{border:1px solid #B1D7EE;width:194px;padding:1px;height:180px;z-index:9998;text-align:center}\r\n";
        stylestr += "#L_calendar-year-month{height:23px;line-height:23px;z-index:9998;background:#F4FBFF }\r\n";
        stylestr += "#L_calendar-year{line-height:23px;width:60px;float:left;z-index:9998;position: absolute;top: 3; left: 45;cursor:default}\r\n";
        stylestr += "#L_calendar-month{line-height:23px;width:48px;float:left;z-index:9998;position: absolute;top: 3; left: 98;cursor:default}\r\n";
        stylestr += "#L_calendar-PrevM{position: absolute;top: 3; left: 22;cursor:pointer}"
        stylestr += "#L_calendar-NextM{position: absolute;top: 3; left: 165;cursor:pointer}"
        ...
        stylestr += "#L_calendar-week li{width:25px;height:20px;float:left;;margin:1px;padding:0px;text-align:center;}\r\n";
        stylestr += "#L_calendar-day ul{list-style:none;margin:0px;padding:0px;}\r\n";
        stylestr += "#L_calendar-day li{cursor:pointer;width:25px;height:20px;float:left;;margin:1px;padding:0px;line-height:22px}\r\n";
        stylestr += "#L_calendar-control{height:25px;z-index:9998;}\r\n";
        stylestr += "#L_calendar-today{cursor:pointer;float:left;width:66px;height:20px;line-height:20px;margin:1px;text-align:center;background:#1195DB}"
        stylestr += "</style>";
    ...
    Today: function()  //Today Button
    {
        ...
        var mm = this.L_TheMonth;
        if (this.L_TheMonth < 10) mm = "0" + this.L_TheMonth;
        today = new Date().getDate();
        if (today < 10) today = "0" + today;
        if (this.InputObject) {
            this.InputObject.value = this.L_TheYear + "-" + mm + "-" + today;
        }
        this.CloseLayer();
    },
    SetDay: function(yy, mm)   //主要的写程序**********
    {
        ...
        /*if (window.top.location.href != window.location.href) {
            ...
            L_DateLayer_Parent = window.self;
        for (var i = 0; i < 39; i++) { this.L_WDay[i] = "" };  //将显示框的内容全部清空
        var day1 = 1, day2 = 1, firstday = new Date(yy, mm - 1, 1).getDay();  //某月第一天的星期几
        for (i = 0; i < firstday; i++) this.L_WDay[i] = this.GetMonthCount(mm == 1 ? yy - 1 : yy, mm == 1 ? 1 : mm - 1) - firstday + i + 1	//上个月的最后几天
        ...
        for (i = 0; i < 39; i++) {
            ...
            if (this.L_WDay[i] != "") {
                if (i < firstday) {
                    da.innerHTML = "<span style=\"color:#ccc;\">" + this.L_WDay[i] + "</span>";
                    month = (mm == 1 ? 1 : mm - 1);
                    day = this.L_WDay[i];
                }
                    ...
                    da.innerHTML = "<span style=\"color:#000;\">" + this.L_WDay[i] + "</span>";
                    month = (mm == 1 ? 1 : mm);
                    day = this.L_WDay[i];
                }
                if (document.all) {
                ...
                da.style.background = (yy == new Date().getFullYear() && month == new Date().getMonth() + 1 && day == new Date().getDate()) ? "#FFD700" : "#F3F3FF";
                if (this.InputDate != null) {
                    if (yy == this.InputDate.getFullYear() && month == this.InputDate.getMonth() + 1 && day == this.InputDate.getDate()) {
                        da.style.background = "#1497DD";
                    }
                }
            }
        }
    ...
    SelectYearInnerHTML: function(strYear) //年份的下拉框
    {
        ...
        if (n < 1000) n = 1000;
        if (n + 26 > 9999) n = 9974;
        var s = "<select name=\"L_SelectYear\" id=\"L_SelectYear\" style='font-size: 12px;margin-left:23px;' "
    ...
    SelectMonthInnerHTML: function(strMonth) //月份的下拉框
    {
        ...
        var s = "<select name=\"L_SelectYear\" id=\"L_SelectMonth\" style='font-size: 12px;margin-left:23px' "
    ...
    SetDate: function() {
        ...
        if (document.all) {
            this.GetDateLayer().document.getElementById("L_calendar").style.width = "194px";
            ...
            this.GetDateLayer().document.getElementById("L_calendar").style.height = "180px"
            DateLayer.style.width = "194px";
            DateLayer.style.height = "188px";
        }
...
document.writeln('<iframe id="L_DateLayer" name="L_DateLayer" frameborder="0" style="position:absolute;width:195px; height:200px;z-index:9998;display:none;"></iframe>');
var L_DateLayer_Parent=null;
var MyCalendar=new L_calendar();
MyCalendar.NewName="MyCalendar";
MyCalendar.addEventHandler(document, "click", function(e) {
    e = window.event || e;
