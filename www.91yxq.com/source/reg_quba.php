...
//	}
    ...
    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,'','',1,'',$pcid, 'quba');
    if ($result == 'ok') {
        ...
        $url = 'http://gameapi.24quba.net/api/game.asp?ID='.$pcid.'&AccountID='.$userId.'&AccountName='.$userName.'&act=ReturnAccount';
        ...
        if ($res != 1) {
            ...
            file_put_contents('24quba.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
