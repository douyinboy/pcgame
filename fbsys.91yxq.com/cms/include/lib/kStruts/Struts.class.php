...
class Struts
{
				...
				public $config = NULL;
				public $request = NULL;
				public $action_cfg = NULL;

				public function Struts( &$STRUTS_CONFIG, &$IN )
				{
								global $db_config;
								global $db;
								global $SYS_ENV;
								...
								if ( ( !isset( $action_cfg['db'] ) || $action_cfg['db'] ) && ( !isset( $db ) || !is_resource( $db ) ) )
								{
				...
				public function tpl_assign( $_key, $_value )
				{
								if ( isset( $this->TPL ) )
								{
				...
				public function &loadActionForm( &$ActionMapping, $action_cfg )
				{
								...
								foreach ( $this->config['form-beans'][$formBeanName]['form-property'] as $key => $var )
								{
												if ( is_array( $var ) )
												{
																if ( isset( $var['validator'] ) && !$FormBean->checkData( $this->request[$key], $var['validator'] ) )
																{
				...
				public function loadAction( &$ActionMapping, $action_cfg, &$ActionForm )
				{
				...
				public function getActionClassName( $actionNS )
				{
...
class Action
{
				...
				public function execute( &$ActionMapping, &$ActionForm, &$IN, &$TPL )
				{
				}
...
class ActionForm
{
				...
				public function &validate( &$ActionMapping, &$IN )
				{
				}

				public function checkData( $data, $validator )
				{
...
class ActionMapping
{
				...
				public $TPL = NULL;
				public $strutsConfig = NULL;
				public $actionErrors = NULL;
				public $sId = NULL;
				public $_parameter = array( );

				public function ActionMapping( &$STRUTS_CONFIG, &$TPL )
				{
				...
				public function doHeaderAction( $url_forward )
				{
				...
				public function getParams( $_sep = "&amp;" )
				{
								$params = "";
				...
				public function doForwardAction( $url_forward, $jumpMsg = "", $jumpTime = 1 )
				{
				...
				public function doForward( $tplname )
				{
...
class ActionErrors
{
				...
				public function add( $error_scope, $error_msg, $params = array( ) )
				{
								global $_LANG_MESSAGES;
