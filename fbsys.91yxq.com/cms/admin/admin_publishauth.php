...
（添加 复制 转移 删除 创建虚链接） 本来就是可以用一个编辑权限来概括*/
	...
	case 'add_submit':
		...
		if($pubAuth->add()) { 
			fgoto("view", 'add_publishadmin_ok');

		} else {
			fgoto("view", 'add_publishadmin_fail');
	...
	case 'edit_submit':
		...
		if($pubAuth->update($IN[pId])) { 
			fgoto("view", 'edit_publishadmin_ok');

		} else {
			fgoto("view", 'edit_publishadmin_fail');
	...
	case 'del':
		...
		if($pubAuth->del($IN[pId])) {
			fgoto("view", 'del_publishadmin_ok');
			
		} else
			fgoto("view", 'del_publishadmin_fail');
