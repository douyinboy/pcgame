	...
	function doCleanCode(code) {
        ...
        code = code.replace(/<p([^>])*>( )*\s*<\/[Pp]>/gi,"")
        // removes all <span> </span> tags 
        code = code.replace(/<span([^>])*>( )*\s*<\/[Ss][Pp][Aa][Nn]>/gi,"") //删除 <span ......>  </span>
        code = code.replace(/<span([^>])*>/gi,"") //删除 <span ......>
        code = code.replace(/<\/span([^>])*>/gi,"") //删除 </span>
        // removes <b> </b>
        code = code.replace(/<b>/gi,"") //删除 <b> 粗体字效果
        code = code.replace(/<\/b>/gi,"") //删除 </b>

        // 处理<p> 段首空两格 先删除<p ...>中间的废代码，然后清除空格，再加空格
        code = code.replace(/<p([^>])*>/gi,"<P>") //替换 <p .... >为<p>
        code = code.replace(/<p>　　( ){0,}( ){0,}( ){0,}/gi,"<P>") //删除段首空格，解决空格多的问题
        code = code.replace(/<p>( ){1,}/gi,"<P>") //同上
        code = code.replace(/<p>/gi,"<P>　　") //加空格

        // 处理<br> 段首空两格,有时可能出现的情况
        code = code.replace(/<br>　　( ){0,}( ){0,}( ){0,}/gi,"<BR>") //删除段首空格，解决空格多的问题
        code = code.replace(/<br>( ){1,}/gi,"<BR>") //同上
