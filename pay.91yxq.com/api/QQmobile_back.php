<?php
$fhandle=fopen(dirname(dirname(__DIR__)) . "/home/91yxq/logs/QQmobile_back.log","a");
...
require_once("../include/pay_funcs.php");
	...
	if ($row->succ) {
		$sql_wx="select id,b_flag from pay_QQmobile where orderid='{$_REQUEST['orderid']}'";
		...
		if ( $row_wx->b_flag) {
		 header("location:http://pay.91yxq.com/pay_to_show.php?orderid=".$_REQUEST['orderid']);exit;
			...
			$sql_u="update pay_QQmobile set b_flag=1,sync_date=now(),succ=1 where orderid='{$_REQUEST['orderid']}'";
			mysql_query($sql_u);
			pay_gameb("pay_QQmobile",$row_wx->id,$_REQUEST['orderid']);
		}
	}
