...
switch($IN[o]) {
	...
	case 'add_submit':
		...
		if($keywords->add()) {
			...
			fgoto('view', 'add_keywords_ok');
		
		} else
			fgoto('view', 'add_keywords_fail');

		break;
	case 'edit':
		if(empty($IN[kId])) fgoto('view');
	...
	case 'edit_submit':
		...
		if($keywords->update($IN[kId])) {
			clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
			fgoto('view', 'edit_keywords_ok');
		
		} else
			fgoto('view', 'edit_keywords_fail');
		break;
	case 'del':
		if(empty($IN[kId])) fgoto('view');
		...
		if($keywords->del($IN[kId])) {
			clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
			fgoto("view", 'del_keywords_ok');
		
		} else
			fgoto("view", 'del_keywords_fail');
