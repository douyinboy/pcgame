...
switch($IN['o']) {
	...
	case 'add_submit':
		...
		if($psn->add()) { 
			...
			fgoto("view", 'add_psn_ok');
		} else	
			fgoto("view", 'add_psn_fail');
		...
		if(empty($IN[PSNID])) fgoto('view');
 		if(!empty($IN[Name]) && !empty($IN[psnType])) {
			...
			if($psn->update($IN[PSNID])) { 
				...
				$cache->makeCache('psn');
				$iWPC->clearALLNodeInfo();
				fgoto("view", 'edit_psn_ok');
			} else	
				fgoto("view", 'edit_psn_fail');
	...
	case 'del':
		if(empty($IN[PSNID])) fgoto('view');
			if($psn->del($IN[PSNID])) {
				...
				$cache->makeCache('psn');
				$iWPC->clearALLNodeInfo();
				fgoto("view", 'del_psn_ok');		
			} else
				fgoto("view", 'del_psn_fail');
