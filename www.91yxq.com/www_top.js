...
(function(window){
    ...
    function loadHtml(){
        var $TopHtml = $(
            ...
            '<ul>'+
            '<li>'+
            '<a href="http://www.918ent.com/hys/index.html" target="_blank">'+
            ...
            '<a href="http://www.918ent.com/turn.php?act=reg" class="reg_btn" target="_blank">注册</a>'+
            '</div>'+
            //'<!--登录后用户信息-->'+
            '<div id="top_username_div" class="fr min_user_info" style="display:none;">'+
            '<a >欢迎您，</a>'+
            '<a id="top_username" href="http://www.918ent.com/user.php"><<用户名>></a>'+
            '<a href="http://www.918ent.com/turn.php?act=logout" class="back">【注销】</a>'+
            '</div>'
        );
        $('#platform_top_div').append($TopHtml);

        //点击弹出登录框(http://www.918ent.com/public/popup_login.js文件中已经绑定事件)
        ...
        function setCookie(cookieName, cookieValue, seconds) {
            ...
            document.cookie = escape(cookieName) + '=' + escape(cookieValue) + (seconds ? ('; expires=' + expires.toGMTString()) : "") + '; path=/; domain=918ent.com;';
        }
        ...
        window.popup_login.top_check_login = function(){
            ...
            //公共头部 显示登录按钮时 检测登录状态
            var url="http://www.918ent.com/api/login_user_status2.php";
            $.ajax({
                type:"get",
                url:url,
                data:{state:1},
    ...
    function loadLO(){
        ...
        lo.setAttribute('src','http://www.918ent.com/public/popup_login.js');
    ...
    function loadJQ(){
        ...
        jQ.setAttribute('src','http://image.918ent.com/js/jquery-1.10.2.min.js');
