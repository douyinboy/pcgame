		...
		if(!empty($IN['tName'])) {
			...
			if($content_table->addTable($IN['basedTableID'])) {
				...
				*/
				...
				//exit;
				fgoto("edittable&TableID={$content_table->db_insert_id}");
			}
			
		} else {
			fgoto('viewtable', 'addtable_name_null');
		}
		break;
	case 'edittable':
		if(!empty($IN[TableID])) {
	...
	case 'edittable_submit':
		...
		if( $IN['TableID']!='') {
			...
			if($content_table->updateTable($IN['TableID'])) {
				...
				$cache->makeCache('content_model');	
				clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
				fgoto("edittable&TableID={$IN['TableID']}", 'edittable_submit_ok');
			} else {
				fgoto("edittable&TableID={$IN['TableID']}", 'edittable_submit_fail');
			}
	...
	case 'deltable':
		...
		//$content_table->delTable($IN['TableID']);exit;
			if($content_table->delTable($IN['TableID'])) {
				...
				$cache->makeCache('content_model');	
				clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
				fgoto("viewtable", 'deltable_ok');
			} else	fgoto("viewtable", 'deltable_fail');
		...
		if( $IN[data][FieldName]!='' &&  $IN[data][FieldTitle]!=''&& $IN[data][FieldType]!='' && $IN['TableID']!='') {
			//echo $IN[data][FieldName];
				...
				if($content_table->addField($IN[TableID], $IN[data])) {
					...
					$cache->makeCache('content_model');	
					clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
					fgoto("edittable&TableID={$IN['TableID']}", 'table_add_ok');
									
				} else {
					fgoto("edittable&TableID={$IN['TableID']}", 'table_add_fail');
				
				}
			
		} else {
			fgoto("addfield&TableID={$IN['TableID']}", 'table_add_data_null');
	...
	case 'editfield_submit':
		...
		if( $IN[data][FieldName]!='' &&  $IN[data][FieldTitle]!=''&& $IN[data][FieldType]!='' && $IN['TableID']!='' && $IN['ContentFieldID']!='') {
				...
				if($content_table->editField($IN['ContentFieldID'], $IN[data])) {
					...
					$cache->makeCache('content_model');	
					clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
					fgoto("edittable&TableID={$IN['TableID']}", 'table_editfield_ok');
									
				} else {
					fgoto("edittable&TableID={$IN['TableID']}", 'table_editfield_fail');
				
				}
			
		} else {
			fgoto("editfield&TableID={$IN['TableID']}", 'table_editfield_data_null');
	...
	case 'delfield':
		if($IN[ContentFieldID] != '') {
			if($content_table->delField($IN[ContentFieldID])) {
				...
				$cache->makeCache('content_model');	
				clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
				fgoto("edittable&TableID={$IN['TableID']}", 'table_delfield_ok');
			
			} else 
				fgoto("edittable&TableID={$IN['TableID']}", 'table_delfield_fail');
		}
		break;
	case 'indexField':
		if($IN[ContentFieldID] != '') {
			if($content_table->indexField($IN[ContentFieldID])) {
				...
				$cache->makeCache('content_model');	
				clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
				fgoto("edittable&TableID={$IN['TableID']}", 'table_indexfield_ok');
			
			} else 
				fgoto("edittable&TableID={$IN['TableID']}", 'table_indexfield_fail');
		}
		break;
	case 'delFieldIndex':
		if($IN[ContentFieldID] != '') {
	...
	case 'orderfield_submit':
		if(!empty($IN[FieldString])) {
			...
			if($content_table->OrderField($Fields)) {
				...
				$cache->makeCache('content_model');	
				clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
				fgoto("edittable&TableID={$IN['TableID']}", 'table_reOrdefield_ok');
			} else
				fgoto("edittable&TableID={$IN['TableID']}", 'table_reOrdefield_fail');
		//debug($Fields);
	...
	case 'field_listdisplay':
		if($IN['ContentFieldID']!='') {
				if($content_table->EnableFieldListDisplay($IN['ContentFieldID'], $IN[display])) {
					...
					$cache->makeCache('content_model');	
					clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
					fgoto("edittable&TableID={$IN['TableID']}");
