...
switch($IN['o']) {
	...
	case 'add_submit':
		...
		if($tpl_vars->add()) { 
			...
			fgoto("view", 'add_tpl_vars_ok');
		} else	
			fgoto("view", 'add_tpl_vars_fail');
		break;
	...
	case 'edit_submit':
		...
		if($tpl_vars->update($IN[Id])) { 
			clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
			fgoto("view", 'edit_tpl_vars_ok');
		} else	
			fgoto("view", 'edit_tpl_vars_fail');
	...
	case 'del':
		if(empty($IN[Id])) fgoto('view');
			...
			clearDir(SYS_PATH.'sysdata/cache/','index.html;.htaccess') ;
				fgoto("view", 'del_tpl_vars_ok');		
			} else
				fgoto("view", 'del_tpl_vars_fail');
