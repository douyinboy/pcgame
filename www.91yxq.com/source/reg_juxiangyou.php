...
//	}
    ...
    if ($result == 'ok') {
        ...
        $url = 'http://i.juxiangyou.com/Game/Api/registerTestPort?tokenID='.$adid.'&recordID='.$pcid.'&accessCode='.$userId.'&accessKey='.$keycode;

        $res = get_curl($url);
        $res2 = json_decode($res, true);
        if ($res2['access_token'] != 'ACCESS_TOKEN') {
            exit('聚享游绑定失败' . $res2['result'] . '----' . $res2['message']);
        } else {
            file_put_contents('juxiangyou.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
