...
switch($IN['o']) {
	...
	case 'add_submit':
		...
		if($dsn->add()) { 
			...
			fgoto("view", 'add_dsn_ok');
		} else	
			fgoto("view", 'add_dsn_fail');
	...
	case 'edit_submit':
		...
		if(!empty($IN[Name])) {
			...
			if($dsn->update($IN[DSNID])) { 
				...
				//$cache->makeCache('dsn');
				//$iWPC->clearALLNodeInfo();
				fgoto("view", 'edit_dsn_ok');
			} else	
				fgoto("view", 'edit_dsn_fail');
	...
	case 'del':
		if(empty($IN[DSNID])) fgoto('view');
			if($dsn->del($IN[DSNID])) {
				...
				//$cache->makeCache('dsn');
				//$iWPC->clearALLNodeInfo();
				fgoto("view", 'del_dsn_ok');		
			} else
				fgoto("view", 'del_dsn_fail');
