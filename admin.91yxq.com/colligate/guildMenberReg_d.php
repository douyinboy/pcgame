...
$agent_id =trim($_REQUEST["agent_id"])+0;
...
$data=$hj=array();
//注册人数
$sql="SELECT count(id) as reg,placeid,from_unixtime(reg_time,'%Y-%m-%d') as rdate FROM ".USERDB.".".REGINDEX.$year." where agent_id=".$agent_id." and reg_time >=unix_timestamp('$date1 00:00:00') and reg_time<=unix_timestamp('$date2 23:59:59') $game  group by placeid,rdate order by game_id ASC ";
$res=$db->find($sql); 
foreach($res as $v){
    $data[$v['placeid']][$v['rdate']]=$v['reg'];
    $hj[$v['rdate']]+=$v['reg'];  //日期 【合计】
    $hj[$v['placeid']]+=$v['reg']; //游戏 【合计】
}
$sql="SELECT id,name FROM ".PAYDB.".".GAMELIST." WHERE is_open=1 order by id desc ";
$game_list_arr = $db ->find($sql);
//公会成员
$sql=" select * from ".GUILDMEMBER." where agent_id=".$agent_id." and  	state=1 ";
$guildmen=$db ->find($sql);
foreach ($guildmen as $v){
    $guildmenber[$v['site_id']]=$v['author'];
}
?>
...
<?php
  ...
  foreach($data as $site_id=>$v) {  ?>
    <tr class="<?php if ($j%2==0) echo 'trOdd'; else echo 'trEven'; $j++;?>" style="text-align: center;">
        <td><?=empty($guildmenber[$site_id]) ? "公会会长":$guildmenber[$site_id]?></td>
        <?php
            $d=$date1;
            while($d<=$date2) { ?>
        <td width="30"><?=$v[$d]+0?></td>
            <?php $d=date('Y-m-d',strtotime($d)+24*3600); } ?>
        <td width="30" ><?=$hj[$site_id]+0?></td>
        <?php } ?>
    </tr>
<?php } else { ?>
