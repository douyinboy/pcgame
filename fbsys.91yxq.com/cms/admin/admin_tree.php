	...
	case 'publish':
		...
		if(1 != $s['sGIsAdmin']){
			foreach($NodeInfo as $key=>$var){
				if($NodeInfo[$key]['PermissionInherit']==1) continue; 
				if(!(in_array($s[sGId],explode(',',$NodeInfo[$key]['PermissionReadG'])))){
					unset($NodeInfo[$key]);
				}
			}
		}
	...
	case 'publish_xml':
		 ...
		 */
			...
			if(1 != $s['sGIsAdmin']){
				foreach($NodeInfo as $key=>$var){
					if($NodeInfo[$key]['PermissionInherit']==1) continue; 
					$platform = array(20,659,1917,1938);
					if(!in_array($NodeInfo[$key]['ParentID'],$platform)) continue;  // NodeID=20 为91wan 此处只限制了官网列表，不限制子栏目
					if(!(in_array($s[sGId],explode(',',$NodeInfo[$key]['PermissionReadG'])))){
						unset($NodeInfo[$key]);
					}
				}
