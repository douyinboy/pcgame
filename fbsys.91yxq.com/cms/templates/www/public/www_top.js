...
(function(window){
	...
	function loadHtml(){
		var $TopHtml = $(
			...
			'<div class="min_game_down">'+
				...
				'<div class="min_game_list" style="display:none">'+
					'<ul>'+
						<cms action="LIST" return="list" NodeID="4" where="c.PlatformId=1 and c.ServiceStatus=1" />
						<loop name="list" var="var" key="key" >
						'<li>'+
							'<a href="[$var.GameWeb]" target="_blank">'+
								'<span class="name">[$var.GameName]</span>'+
								'<i class="imgpq <if test="$var.GameTag==1">min_ic_new<elseif test="$var.GameTag==2">min_ic_hot</if>"></i>'+
							'</a>'+
						'</li>'+
						</loop>
					'</ul>'+
			...
			'<div id="top_login_div" class="fr login">'+
				...
				'<a href="[$91yxq_url]/turn.php?act=reg" class="reg_btn" target="_blank">注册</a>'+
			'</div>'+
			//'<!--登录后用户信息-->'+
			'<div id="top_username_div" class="fr min_user_info" style="display:none;">'+
				'<a >欢迎您，</a>'+
				'<a id="top_username" href="[$91yxq_url]/user.php"><<用户名>></a>'+
				'<a href="[$91yxq_url]/turn.php?act=logout" class="back">【注销】</a>'+
			'</div>'
		);
		$('#platform_top_div').append($TopHtml);
		
		//点击弹出登录框([$91yxq_url]/public/popup_login.js文件中已经绑定事件)
		...
		window.popup_login.top_check_login = function(){
			...
			//公共头部 显示登录按钮时 检测登录状态
			var url="[$91yxq_url]/api/login_user_status2.php";
			$.ajax({
				type:"get",
				url:url,
				data:{state:1},
	...
	function loadLO(){
		...
		lo.setAttribute('src','[$91yxq_url]/public/popup_login.js');
