...
if($_POST['action']){//注册
    ...
    $key = '4247cf561c64cd8331a52e8b8addd9e9';
    $plaid = 236;

    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,urlencode($adid),'',1,'',$pcid, 'ledouwan');
    if ($result == 'ok') {
        ...
        $url = 'http://www.ledouwan.com/index.php?c=tgame&a=v3api&luid='.$pcid.'&lwid='.$adid.'&ltype=pagegame&key='.$key.'&plaid='.$plaid.'&guid='.$userId;

        $res = get_curl($url);
        $res2 = json_decode($res, true);

        if ($res2['status'] == 0) {
            exit('乐豆玩绑定失败----' . $res2['msg']);
        } else {
            file_put_contents('ledouwan.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
