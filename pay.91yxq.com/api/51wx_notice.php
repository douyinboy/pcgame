<?php
$fhandle=fopen(dirname(dirname(__DIR__)) . "/home/91yxq/logs/51wx_notify.log","a");
...
if($state == '1'){
	...
	$succ=1;
	$sql="select id,b_flag from pay_51wx where orderid='{$sdcustomno}'";
	$res=mysql_query($sql);
	...
	if ($row) {
		...
		$sql="update pay_51wx set sd51no='{$sd51no}',b_flag=1,sync_date=now(),succ=1 where orderid='{$sdcustomno}'";
		mysql_query($sql);
		if (!$row->b_flag) {
			pay_gameb("pay_51wx",$row->id,$sdcustomno);
		}
	}else {
		mysql_query("UPDATE `pay_orders` SET sync_date=".time().",succ=1 WHERE orderid='{$sdcustomno}'");
		$sql="insert into pay_51wx (customerid,sd51no,orderid,ordermoney,realmoney,pay_date) values(
