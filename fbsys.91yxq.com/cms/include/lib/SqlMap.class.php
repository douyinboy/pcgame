...
class SqlMap
{
				...
				public $_db = NULL;
				public $_basefile = NULL;
				public $_pojo = array( );
				public $_sqlmap_select = array( );
				public $_sqlmap_update = array( );
				public $_sqlmap_delete = array( );
				public $_sqlmap_insert = array( );
				public $_contextIsLoaded = false;
				public $_debug = false;
				
				public function SqlMap( $_context_bundle, $_auto_start = false )
				{
			    ...
			    global $db,$db_config;
								if ( is_array( $_data ) )
								{
												...
												{
																$this->_pojo[$key] = $db->escape_string( $var );
												}
								}
								else
								{
												$this->_pojo[$_data] = $db->escape_string( $_val );
								}
				}
				...
				public function filterData( &$IN, $_prefix = "data_" )
				{
								if ( !is_array( $IN ) )
								{
												return false;
				...
				public function _processQueryStr( $_sqlmap, $_escape_str = false )
				{
					global $db,$db_config;
								if ( is_array( $_sqlmap ) )
								{
								...
								if ( preg_match_all( "/#([a-zA-Z0-9_]+)#/isU", $sql, $matches ) )
								{
												foreach ( $matches[0] as $key => $var )
												{
																if ( $_escape_str )
																{
																				$sql = str_replace( $var, $db->escape_string( $this->_pojo[$matches[1][$key]] ), $sql );
																}
																else
																{
				...
				public function insert( $_queryid, $_pojo = NULL )
				{
					global $db,$db_config;
								$escape_string = false;
								if ( !empty( $_pojo ) )
				...
				public function dataInsert( $_queryid )
				{
					global $db,$db_config;
								if ( !isset( $this->_sqlmap_insert[$_queryid] ) )
								{
				...
				public function dataReplace( $_queryid )
				{
					global $db,$db_config;
								if ( !isset( $this->_sqlmap_insert[$_queryid] ) )
								{
				...
				public function update( $_queryid, $_pojo = NULL )
				{
					global $db,$db_config;
								$escape_string = false;
								if ( !empty( $_pojo ) )
				...
				public function settingUpdate( $_queryid )
				{
					global $db,$db_config;
								if ( !isset( $this->_sqlmap_update[$_queryid] ) )
								{
				...
				public function dataUpdate( $_queryid )
				{
					global $db,$db_config;
								if ( !isset( $this->_sqlmap_update[$_queryid] ) )
								{
				...
				public function del( $_queryid, $_pojo = NULL )
				{
								global $db_config;
								$escape_string = false;
								if ( !empty( $_pojo ) )
				...
				public function dataDel( $_queryid )
				{
					global $db,$db_config;
								if ( !isset( $this->_sqlmap_delete[$_queryid] ) )
								{
				...
				public function queryForObject( $_queryid, $_pojo = NULL )
				{
								global $db,$db_config;
								$escape_string = false;
								if ( !empty( $_pojo ) )
				...
				public function queryForList( $_queryid, $_pojo = NULL )
				{
								global $db,$db_config;
								$escape_string = false;
								$data = array( );
				...
				public function queryForSettingMap( $_queryid, $_pojo = NULL )
				{
								global $db,$db_config;
								$escape_string = false;
								$data = array( );
