...
; (function($) {
	$.fn.extend({
		imgChange: function(o) {
			...
			//初始化
			if(size) {
				o.showTxt&&_pp.after("<a class='txt' href='" + _img[0].attributes['data-url'].nodeValue + "'>" + _img[0].alt + "</a>");
				function _sInit() {
					_p[0].style.cssText = 'margin: 0;padding: 0;position: absolute;left:0;top:0;'+sizeCss+':999999px';
					_pp[0].style.cssText = 'overflow: hidden;position: relative;'+sizeCss1+':'+350+'px';
			...
			switch (o.effect) {
				case 'wfScroll':
					if(size < v) return;
					_sInit();
					var _scrollSize = totalSize(_self);
					_pp[0].style[sizeCss]=(o.wrapSize||totalSize(_self.slice(0, v)))+'px';
					_p[0].innerHTML+=_p[0].innerHTML;
					break;
			...
			function marquee(){var _animCss=parseInt(_p[0].style[animCss])-o.steps; _p[0].style[animCss]=-_animCss>_scrollSize?0:_animCss+'px'}
			//统计对象的宽度或高度
			...
			function fadeAB() {
				...
				switch (o.effect) {
					...
					//切换标题
					o.showTxt&&(_pp.next().html(_img[index].alt)[0].href = _img[index].attributes['data-url'].nodeValue);
					nowIndex = index;
					//完成的函数
