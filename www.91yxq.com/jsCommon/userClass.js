    ...
    if(logFlag){
        ...
        if(ucGID("usrAccount")){
            ...
            if(qq_nick&&logUsrName.indexOf('@qq')>0){
                ucGID("usrAccount").innerHTML='<img src="http://image.6qwan.com/imgCommon/qqico.gif" width="16" height="16" />'+qq_nick+' - '+logUsrName;
            }else{
...
function LogoutWithGoto(aBackFlag){
    if (aBackFlag=="1"){
        ...
        gotoUrl = "http://www.6qwan.com/";
    }
    LogoutUrl = "http://www.6qwan.com/api/check_login_user.php?act=logout" + "&backurl=" + escape(gotoUrl) ; // 这里要完善为encodeUrl
                ...
                case '10':
                    if (backCurPage=="1"){  //默认为登录成功后返回到当前页面
                        ...
                        if (ucGID("logGotoUrl") && ucGID("logGotoUrl").value.length > 7){//指定需要跳转的页面地址；
                            ...
                            gotoUrl = "http://www.6qwan.com/fyws/dlq/index.html";
                        }
                    }
...
function sendLogDataBysid(aFrmLog) {
    ...
    frmActionUrl = 'http://www.6qwan.com/index.php?act=gamelogin&game_id='+game_id+'&server_id='+server_id;
