...
if (!isset($_COOKIE['login_name'])) {
    $url = 'http://www.' . DOMAIN . '.com/dpsc/wd/login.php';
    header("Location:".$url);
    exit;
}
...
$sql = "SELECT ServerId, ServerName, ServerStatus FROM  91yxq_publish.91yxq_publish_6 WHERE GameId = 2 ORDER BY ServerId DESC";
                        ...
                        <?php if (!empty($server_list)): ?>
                            ...
                            <div class="sever_box">
                                <div class="sever_cont">
                                    <a href="../../main.php?act=gamelogin&uid=<?php echo $uid_arr['uid'] ?>&wd=1&isclient=1&token=895DA1201DE8469D9B79845E3836EF9C&game_id=2&server_id=<?php echo $val['ServerId'] ?>&sid=<?php echo $val['ServerId'] ?>"><p class="sever_name" for="sele_label2"><?php echo $val['ServerName']?>【<?php echo $val['ServerStatus']?>】</p></a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                        <?php endif; ?>
