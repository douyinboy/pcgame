<?php
$fhandle=fopen(dirname(dirname(__DIR__)) . "/home/91yxq/logs/sync_99bill_szx.log","a");
...
if($payResult!="10" || strtoupper($signMsg)!=strtoupper($merchantSignMsg)){
	$fhandle=fopen(dirname(dirname(__DIR__)) . "/home/91yxq/logs/sync_99bill_szx1.log","a");
	fwrite($fhandle,date("Y-m-d H:i:s")."	".$_SERVER["HTTP_CDN_SRC_IP"]."	".$_SERVER["REQUEST_URI"]."\r\n");
	fclose($fhandle);
...
if ($row) {
	if ($payResult=="10") {
		...
		$sql="update pay_99bill_szx set b_flag=1,dealId='$dealId',dealTime='$billOrderTime',payAmount='$payAmount',payResult='$payResult',signMsg='$signMsg',sync_date=now(),succ='1' where orderid='$orderId'";
		mysql_query($sql);
		if (!$row->b_flag) {
	...
	$fhandle=fopen(dirname(dirname(__DIR__)) . "/home/91yxq/logs/sync_99bill_szx2.log","a");
	fwrite($fhandle,date("Y-m-d H:i:s")."	".$_SERVER["HTTP_CDN_SRC_IP"]."	".$_SERVER["REQUEST_URI"]."\r\n");
	fclose($fhandle);
echo "<result>1</result><redirecturl>http://pay.91yxq.com/pay_to_show.php?orderid=".$orderId."</redirecturl>";
?>