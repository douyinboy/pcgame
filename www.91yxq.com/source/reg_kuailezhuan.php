...
//	}
    ...
    if ($result == 'ok') {
        ...
        $userId   = $result;
        $userName = $login_name;
        $keycode = md5($key . $adid . $pcid . $userId . $userName . $key);

        $url = 'http://www.lezhuan.com/reannal.php?adID='.$adid.'&leID='.$pcid.'&idCode='.$userId.'&idName='.$userName.'&sign='.$keycode;

        $res = get_curl($url);
        $res2 = json_decode($res, true);
        if ($res2['result'] != 1) {
            exit('快乐赚绑定失败' . $res2['result'] . '----' . $res2['message']);
        } else {
