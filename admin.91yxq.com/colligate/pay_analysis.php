...
/*-------------------充值分时处理--------------------------------------*/
$sql="select sum(`money`) as sm,from_unixtime(unix_timestamp(`pay_date`),'%k') as ph from ".PAYDB.".".PAYLIST." $s and game_id !=0 AND server_id !=0 group by ph";
$pay_m_arr =$db ->find($sql);
$data = array();
for ($i = 0; $i <= 23; $i++) {
        $data[$i] = array('money'=>0);
}
$sum_money=0;
foreach ( $pay_m_arr as $key=>$val) {
    $reg_h = $val['ph'];
        if ( $val['sm']>= $money1 && $val['sm']<= $money2 ) {
        $data[$reg_h]['money'] = $val['sm'];
        $sum_money+=$val['sm'];
    }
}
...
<TABLE class=table_list border=0 cellSpacing=1 cellPadding=2>
  <TBODY>
    <TR class=table_list_head>
        <TD width="139" noWrap>日期时段(小时)/充值金额 </TD>
	<?php foreach( $data as $key=>$value ) { ?>
        <TD width="17"><?=$key?>点</TD>
	<?php } ?>
  </TR>
  <TR class=<?php if ($i%2==0) echo 'trOdd'; else echo 'trEven';$i++;?>>
    <TD noWrap>合计：<?=$sum_money?></TD>
	<?php foreach( $data as $key=>$value ) { ?>
    <TD noWrap><?=$value['money']?></TD>
    <?php } ?>
  </TR>
</TBODY></TABLE>
...
<script type="text/javascript" >
    ...
    chart = new Highcharts.Chart({
        ...
        tooltip: {
            formatter: function() {
            return '<b>'+ this.series.name +'</b><br/>'+this.x +'时: '+ this.y +'金额';
            }
        },
        legend: {
            layout: 'vertical',
