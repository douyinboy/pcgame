...
switch($IN[o]) {
	...
	case 'fieldReplace_submit':
		$table_name = $db_config['table_pre'].$db_config['table_content_pre'].'_'.$IN['TableID'];
 		foreach($IN['Fields'] as $var) {
			if(!empty($IN['NodeIDs'])) {
				...
				//UPDATE cmsware_content_1 c,cmsware_content_index i set c.Keywords='b' WHERE c.ContentID=i.ContentID and i.NodeID='1'
				if(empty($ids)) {
		...
		if($result) showmessage('field_replace_ok', $referer);
		else showmessage('field_replace_fail', $referer);
		break;
	default:
