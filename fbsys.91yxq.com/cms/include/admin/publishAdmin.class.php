...
class publishAdmin extends iData
{
				...
				public function indexEdit( $IndexID )
				{
								global $table;
				...
				public function linkExists( $ContentID, $NodeID )
				{
								global $table;
								global $db;
				...
				public function getResyncIndexLimit( $NodeID, $start = 0, $offset = 15, $State = "!= -1", $Type = "" )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function restore( $IndexID )
				{
								$this->flushData( );
				...
				public function destroy( $IndexID )
				{
								global $table;
								global $iWPC;
								global $db_config;
				...
				public function destoryResource( $IndexID )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function empty_recycle_bin( $NodeID )
				{
								global $table;
								global $iWPC;
								global $db_config;
				...
				public function getResourceInfo( $IndexID, $Category )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function getAllIndex( $NodeID, $State, $field = "*" )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function getLinkState( $IndexID )
				{
								global $table;
								global $iWPC;
								global $db;
				...
				public function top( $IndexID, $Num )
				{
								$this->flushData( );
				...
				public function pink( $IndexID, $Num )
				{
								$this->flushData( );
				...
				public function sortIt( $IndexID, $Num )
				{
								$this->flushData( );
				...
				public function move( $IndexID, $targetNodeID )
				{
				...
				public function copyTo( $IndexID, $targetNodeID )
				{
								global $iWPC;
								global $sys;
				...
				public function createLink( $IndexID, $targetNodeID )
				{
								global $iWPC;
								$IndexInfo = $this->getIndexInfo( $IndexID );
								$this->flushData( );
				...
				public function createIndexLink( $IndexID, $targetNodeID )
				{
								global $iWPC;
								$IndexInfo = $this->getIndexInfo( $IndexID );
								$this->flushData( );
				...
				public function indexDel( $IndexID )
				{
								global $table;
								global $db;
								$this->unpublish( $IndexID );
				...
				public function contentHaveIndex( $ContentID )
				{
								global $table;
								global $db;
				...
				public function getIndexLimit( $NodeID, $start = 0, $offset = 15, $State = "!= -1", $Type = "" )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function getIndexRecordNum( $NodeID, $State = "!= -1", $Type = "" )
				{
								global $table;
								global $db;
								if ( !empty( $Type ) )
				...
				public function getIndexInfo( $IndexID, $field = "*" )
				{
								global $table;
								global $db;
				...
				public function getContentInfo( $IndexID )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function getPublishInfo( $IndexID )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function editor_getContentInfo( $IndexID )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function getUnPublishLimit( $NodeID, $start, $offset )
				{
								global $db;
								global $table;
				...
				public function getPublishLimit( $NodeID, $start, $offset )
				{
								global $db;
								global $table;
				...
				public function IsRecordExists( $params )
				{
								global $db_config;
								global $iWPC;
								global $db;
				...
				public function contentAdd( $NodeID, $IndexInfo )
				{
								global $db_config;
								global $iWPC;
								global $IN;
				...
				public function imageContentAdd( $NodeID, $PublishDate )
				{
								global $db_config;
								global $iWPC;
								$NodeInfo = $iWPC->loadNodeInfo( $NodeID );
				...
				public function contentEdit( $IndexID, $IndexInfo = "" )
				{
								global $db_config;
								global $iWPC;
								$indexInfo = $this->getIndexInfo( $IndexID );
				...
				public function extraUpdate( $ContentID, $eId )
				{
								global $db_config;
				...
				public function publish( $IndexID )
				{
								global $iWPC;
								global $table;
								global $db;
				...
				public function unpublish( $IndexID, $mode = 1 )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function clearPublishedItem( $ContentID, $NodeID, $mode = 1 )
				{
								global $db;
								global $table;
				...
				public function refresh( $IndexID )
				{
								global $iWPC;
								global $table;
								global $db;
				...
				public function refreshIndex( $NodeID, $tplname, $filename )
				{
								global $iWPC;
								global $SYS_ENV;
								$NodeInfo = $iWPC->loadNodeInfo( $NodeID );
				...
				public function fetchIndex( $NodeID, $template_dir = "", $templatefilename = "" )
				{
								global $iWPC;
								global $SYS_ENV;
								global $IN;
				...
				public function publishURL( $IndexID )
				{
								global $db;
								global $db_config;
								global $Plugin;
				...
				public function publishDynamic( $IndexID )
				{
								global $db;
								global $db_config;
								global $Plugin;
				...
				public function publishUpdate( $TableID )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function publishDel( $IndexID, $mode = 1 )
				{
								global $table;
								global $db;
								global $iWPC;
				...
				public function publishMakeHtml( $IndexID )
				{
								global $db;
								global $db_config;
								global $Plugin;
				...
				public function htmlPhotoPublish( &$content )
				{
								global $table;
				...
				public function _htmlPhoto_parseContent( $content )
				{
				...
				public function psnPublish( &$content )
				{
				...
				public function resourcePublish( &$content )
				{
								global $db;
								global $table;
								global $SYS_ENV;
				...
				public function publishAll( &$string )
				{
				...
				public function cmsware_stream_parse( &$string )
				{
				...
				public function getPublishLog( $ContentID, $NodeID )
				{
								global $db;
								global $table;
								global $SYS_ENV;
				...
				public function _resourcePublishing( $ImgArray )
				{
								global $db;
								global $table;
								global $SYS_ENV;
				...
				public function _imgLocalFilter( $img_data )
				{
								global $SYS_ENV;
				...
				public function _output( &$value, $ImgArray, $localImgArray )
				{
								foreach ( $ImgArray as $key => $var )
								{
				...
				public function _publishing( $filename, $content )
				{
								$psn =& $this->beanFactory->getBean( "psn" );
								$patt = "/{PSN:([0-9]+)}([\\S]*)/is";
								...
								/*** log add by 20110720 ***/
								$str=date('Y-m-d H:i:s').'->'.$this->publishInfo[NodeID].'->'.$this->publishInfo[ContentID].'->'.$filename;
								exec("echo ".$str." >> /root/psn_upload_log");
								/*** end ***/
				...
				public function convertPSN2URL( $_str )
				{
								$patt = "/{PSN-URL:([0-9]+)}/is";
				...
				public function getHtmlURL( $publishFileName )
				{
								$patt = "/{PSN-URL:([0-9]+)}([\\S]*)/is";
				...
				public function ReplaceKeywords( $str )
				{
								global $table;
								global $db;
								global $SYS_ENV;
				...
				public function doPregReplace( $_keyword, $_replace, $_str )
				{
								if ( preg_match_all( "/".preg_quote( $_keyword )."/si", $_str, $matches ) )
								{
												...
												else
												{
																...
																//$_str = str_replace( $_keyword, $_replace, $_str );
																$_str = $this->str_replace_limit( $_keyword, $_replace, $_str ,2);
				...
				public function getRelateDoc( $IndexID, $from, $to )
				{
								global $table;
								global $db;
								global $SYS_ENV;
								...
								foreach ( $keywords as $var )
								{
												...
												{
																if ( !empty( $doclist ) && in_array( $relativeLink->fields, $doclist ) )
																{
				...
				public function makeIndexSavePath( $IndexID )
				{
				...
				// Set servers status
				// added by Max 20110504
				public function setServerStatus( $IndexID,$ServerStatus )
				{
								global $table;
								global $db;
                                global $db_config;
								$IndexInfo = $this->getIndexInfo( $IndexID );
								$where = "where ContentID=".$IndexInfo[ContentID];
								$this->flushData( );
								$this->addData( 'ServerStatus', $ServerStatus );
								if ( $this->dataUpdate( $db_config['table_pre'].'content_6', $where ) ) //开服列表模型
								{
									return true;
								}
								else
				...
				public function str_replace_limit($search, $replace, $subject, $limit=2) {
					if (is_array($search)) {
