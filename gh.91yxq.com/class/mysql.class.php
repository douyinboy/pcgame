...
class Mysql{
	...
	/**
     * 获取指定编号的记录.
	 * @param string $table 不带`号的数据库表
     * @param array $field 要获取的记录的数组.
     * @param array $where 条件数组
	 * @param array $limit 限制条数数组
	 * @param array $where_con 条件语句补充 用于非等于判断
	 * return array 一个多维数组
     */
	public function load_array($table, $field = array(), $where = array(), $limit = array(), $where_con = array(), $order_by = array()){
		$text = '';
		foreach ($field as $v) {
			$text .= "$v,";
		}
		...
		if ($limit) {
			...
			unset($v);
			$limit_text = substr($limit_text, 0, -1);
		}
		$con = '';
		if ($where || $where_con) {
			$con = 'WHERE';
		}
        $sql = "SELECT $text FROM $table $con $text2 $order_text $limit_text";
		unset($text, $table, $text2, $limit, $order_by);
        $row = $this->find($sql);
        return $row;
    }
