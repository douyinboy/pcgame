    ...
    public function weekjie() {
        ...
        if($postData['api'] =='add'){
            if(!empty($_SESSION['user']['uid'])){
                ...
                " where pstate=0 and agent_id=".$_SESSION['user']['uid']." group by wsdate,wedate ORDER BY atime DESC ";
        ...
        if($postData['api'] =='detail'){
            $sql=" select wsdate,wedate,game_id,fc,round(sum(pay_amount),2) as paymoney,round(sum(pay_inner),2) as innerpay,round(sum(pay_jie),2) as pay_jie from ".PAYDB.".".GUILDWEEK." where agent_id =".$_SESSION['user']['uid']." and wsdate >=".$postData['wsdate']." and wedate <=".$postData['wedate']." and pstate=1 group by game_id";
