...
switch($IN[o]) {
		...
		case 'content_list': //内容管理 文章列表
			...
			if($sys->session[sGIsAdmin] == 1) {
				...
				/*
				 * 修改用户的审核权限，如果有父用户组，则继承父用户组所有审核权限
				 * added by Max 20110806 13：23
				 */
				$ParentGID = $sys->session[Auth][ParentGID];
				if($ParentGID!=0){
					$GroupState  = $workflow->getAllStateByGroup($NodeInfo[WorkFlow], $ParentGID);
				}else{
			...
			if($sys->session[sGIsAdmin] == 1) {
				...
				$ParentGID = $sys->session[Auth][ParentGID];
				if($ParentGID!=0){
					$TPL->assign('workflowRecord', $workflow->getAllWorkFlowRecordByGroup($NodeInfo[WorkFlow], $ParentGID));
				}else{
