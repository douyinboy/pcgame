...
//	}
    ...
    $result = $u->reg($login_name,$login_pwd,'','','','','',$agent_id,$game_id,$server_id,$placeid,$from_url,urlencode($adid),'',1,'',$pcid, 'shitoucun');
    if ($result == 'ok') {
        $url = "http://api.91yxq.com/api/get_userid.php?user_name=" . urlencode(strtolower($login_name));
        $result = get_curl($url);
        $userId   = $result;
        $userName = $login_name;
        $keycode = strtolower(md5($key . $adid . $userId));

        $url = 'http://www.stcun.com/Interfaces/callback?game_logo='.$adid.'&uid='.$pcid.'&game_uid='.$userId.'&game_username='.$userName.'&sign='.$keycode;

        $res = get_curl($url);
        $res2 = json_decode($res, true);

        if ($res2['status'] != 0) {
            exit('石头村绑定失败----' . $res2['msg']);
        } else {
            file_put_contents('shitoucun.txt', date('Y-m-d H:i:s') . '--' . $pcid . '绑定成功<br />', FILE_APPEND);
        }
    }
