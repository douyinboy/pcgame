...
if (!isset($_COOKIE['login_name'])) {
    $url = 'http://www.' . DOMAIN . '.com/rxhw/wd/login.php';
    header("Location:".$url);
    exit;
}

$serverStatusArr = [0 => '停服', 1 => '维护', 2 => '待开', 3 => '顺畅', 4 => '火爆', 5 => '爆满'];
$sql = "SELECT ServerId, ServerName, ServerStatus FROM  91yxq_publish.91yxq_publish_6 WHERE GameId = 3 AND ServerStatus > 2 ORDER BY ServerId DESC";
                        ...
                        <?php if (!empty($server_list)): ?>
                            ...
                            <div class="sever_box">
                                <div class="sever_cont">
                                    <a href="../../main.php?act=gamelogin&wd=1&game_id=3&server_id=<?php echo $val['ServerId'] ?>"><p class="sever_name" for="sele_label2"><?php echo $val['ServerName']?>【<?php echo $val['ServerStatus']?>】</p></a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                        <?php endif; ?>
